# Contract — character-architecture

## Runtime capability aliases
- `character_bible_creation`
- `character_arc_audit`
- `relationship_state_tracking`
- `voice_fingerprint_design`
...
Use this skill when a work order or user request clearly needs: Create and audit character identity, wants/needs, arcs, relationships, state and voice anchors for production.
...
- Traits must predict behavior under pressure.
- A wound never excuses missing present-tense objective/agency.
- Relationship state changes require witnessed action/evidence, not off-screen assumption.
...
- RelationshipArc
- VoiceFingerprint
- knowledge/leverage map
- state-transition checkpoints
- ensemble overlap findings

## Skill-specific quality checks
- Names-removed dialogue remains attributable in sample lines.
- Arc changes cite specific decisions/events.
- Character knowledge never includes unrevealed information.

## Handoff
scene-construction/dialogue-voice-subtext/character_director.

## Example
A confident medic wants control but needs to trust others; under pressure the same coping strategy that makes her competent also damages her partnership.

## Counterexample
The arc says a character “learns teamwork” only because the final scene describes them as changed.
